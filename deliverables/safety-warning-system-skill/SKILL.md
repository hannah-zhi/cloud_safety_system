...
- 预警详情:
  ...
  - In 预警管理, aggregate rows by `预警名称 + 模块 + 场站 + 位置`. A row represents the full group, while 等级、事件时间、预警时间、状态 use the latest alarm in that group.
  ...
  - The alarm modal should behave like a card for the aggregate group: show multiple time-specific alarm cards in the upper section, and update the lower trend chart when a different alarm card is selected.
  ...
  - Clicking 处理 opens a two-path workflow:
    - First choose `关闭预警` or `下发 SR`.
    - If closing directly, require one of `误报`, `数据异常`, or `其他`; `其他` requires a manual reason.
    - If issuing SR, render an SR form with `SR编号`, `工单编号`, editable `操作指导`, `SR发出日期`, `期望完成时间`, and attachment upload.
