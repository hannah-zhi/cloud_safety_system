...
- Single-station detail:
  ...
  - SOS 安全指数详情清单 defaults to sorting by current SOS ascending. Every column header is clickable and toggles ascending/descending order.
  - SOS 安全指数详情清单 has a sticky translucent header matching the alarm detail table. Only 子系统, 当前 SOS 指数, and 更新日期 are sortable; 风险描述 and 处置建议 are plain text headers. Sort toggles must not cause column width shifts.
