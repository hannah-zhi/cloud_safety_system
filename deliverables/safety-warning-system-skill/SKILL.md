...
- Sidebar menu uses a three-level structure:
  - 一级：全景概览、安全监测、健康管理、运营评估、交易协同、数据分析、数据运营。
  - 二级（安全监测下）：安全预警、风险运营。
  - 三级（安全预警下）：场站总览、风险透视、预警详情。
- Only the active third-level page should use the strong blue selected state. Parent levels stay in expanded state without competing highlight.
- 场站总览:
