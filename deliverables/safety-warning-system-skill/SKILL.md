...
- 预警详情:
  ...
  - Clicking 处理 opens a separate centered processing modal rather than expanding content at the bottom of the detail modal. The processing modal contains a two-path workflow:
    ...
    - For `关闭-站端已处理` alarms, show station-side investigation action and conclusion in the detail card, disable the 处理 button, and do not require any additional processing.
...
- The alarm-detail status filter sits immediately after level, is wide enough for every status option, and the filter bar should wrap instead of showing horizontal scrolling.
